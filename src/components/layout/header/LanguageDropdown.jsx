import { useState,useRef,useEffect } from "react";
import { LANGUAGES } from "@/utils/constants/Languages";
import { RiArrowDownSFill } from "react-icons/ri";
import Button from "@/components/ui/Button";

function LanguageDropdown() {
    const [open, setOpen] = useState(false);
    const [selected, setSelected] = useState(LANGUAGES[0]);
    const ref = useRef(null);
    useEffect(() => {
      const handleClickOutside = (e) => {
        if (ref.current && !ref.current.contains(e.target)) {
          setOpen(false);
        }
      };
      document.addEventListener("mousedown", handleClickOutside);
      return () => {
        document.removeEventListener("mousedown", handleClickOutside);
      };
    }, []);
    return (
        <div>
            <div ref={ref}  onClick={() => setOpen(!open)} className="relative px-3 py-1.5 cursor-pointer rounded-2xl flex items-center gap-2 hover:bg-gray-700">
                <Button variant={'secondary'}>
                    <span className="flex text-[14px] items-center text-white gap-2">
                        {selected.code}
                    </span><RiArrowDownSFill className="text-[18px]"/>
                </Button>
            <div>
                {open && (
                    <ul className="absolute w-70 flex flex-col gap-2 justify-center right-0 top-10 bg-neutral-900 rounded shadow z-10">
                        {LANGUAGES.map((item) => {
                            const Icon = item.icon
                            return(
                                <li
                                key={item.id}
                                onClick={() => {
                                    setSelected(item);
                                    setOpen(false);
                                }}
                                className={`px-3 py-2 cursor-pointer text-white hover:bg-gray-700 flex items-center gap-2`}
                                ><Icon className={` ${selected.id ===item.id ? 'text-yellow': 'text-white'}`}/> {item.title}</li>   
                            )
                        })}
                    </ul>
                )}
            </div>
            </div>
        </div>
    );
}
export default LanguageDropdown;
